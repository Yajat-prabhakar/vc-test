...
//     try {
//       const { data, error } = await supabase.from('medicine_prescriptions').insert(
//         dictatedMedicines.map(medicine => ({
...
//       if (error) {
//         console.error('Supabase Error:', error.message);
//         alert('Failed to save medicines to database: ' + error.message);
//       } else {
//         console.log('Successfully saved to Supabase:', data);
//         alert('Medicines saved successfully!');
//         setDictatedMedicines([]); // Clear after successful save
//       }
//     } catch (error) {
//       console.error('Unexpected error saving medicines:', error);
//       alert('An unexpected error occurred while saving medicines');
//     }
//   };
...
//       const shouldSave = window.confirm(`You have ${dictatedMedicines.length} prescribed medicines. Do you want to save them to the database?`);
//       if (shouldSave) {
//         saveMedicineToSupabase();
//       }
//     }
...
//   const handleUpdateMedicines = (medicines: string[]) => {
//     setDictatedMedicines(medicines);
//   };
...
//               {/* Save Medicines Button - Only show when there are medicines to save */}
//               {dictatedMedicines.length > 0 && (
//                 <button
//                   onClick={saveMedicineToSupabase}
//                   className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors font-medium text-sm"
//                   title={`Save ${dictatedMedicines.length} prescribed medicines`}
//                 >
//                   Save Medicines ({dictatedMedicines.length})
//                 </button>
//               )}
//             </div>
//           </div>
...
import React, { useState, useRef, useEffect } from 'react';
import { Video, VideoOff, Mic, MicOff, Phone, MessageCircle, Send, Stethoscope, AlertCircle, FileText, Pill, Check, Database } from 'lucide-react';
import { useWebRTC } from '../hooks/useWebRTC';
import { useSocket } from '../hooks/useSocket';
import ConversationRecorder from './ConversationRecorder';
import MedicineDictation from './MedicineDictation';
...
const DoctorPage: React.FC = () => {
  ...
  const confirmPrescription = async () => {
    if (dictatedMedicines.length === 0) {
      alert('No medicines to prescribe. Please add medicines first.');
      return;
    }
    ...
    try {
      ...
      if (error) {
        console.error('Supabase Error:', error.message);
        alert('Failed to confirm prescription: ' + error.message);
      } else {
        console.log('Successfully saved prescription:', data);
        alert(`Prescription confirmed! ${dictatedMedicines.length} medicine(s) have been prescribed to ${patientParticipant?.name || 'Patient'}.`);
        setDictatedMedicines([]); // Clear after successful save
        // Optionally close the medicine dictation panel
        setShowMedicineDictation(false);
      }
    } catch (error) {
      console.error('Unexpected error confirming prescription:', error);
      alert('An unexpected error occurred while confirming prescription');
    } finally {
  ...
  const saveConsultationData = async () => {
    ...
    try {
      const consultationData = {
        ...
        status: isCallActive ? 'active' : 'completed',
        duration_minutes: Math.floor(Math.random() * 60), // You might want to track actual duration
      ...
      if (error) {
        console.error('Supabase Error:', error.message);
        alert('Failed to save consultation data: ' + error.message);
      } else {
        console.log('Successfully saved consultation:', data);
        alert(`Consultation data saved successfully for ${patientParticipant?.name || 'Patient'}!`);
      }
    } catch (error) {
      console.error('Unexpected error saving consultation:', error);
      alert('An unexpected error occurred while saving consultation data');
    } finally {
  ...
  const handleToggleMedicineDictation = () => {
    if (showMedicineDictation && dictatedMedicines.length > 0) {
      // Ask user if they want to save before closing
      const shouldSave = window.confirm(`You have ${dictatedMedicines.length} prescribed medicines. Do you want to confirm the prescription before closing?`);
      if (shouldSave) {
        confirmPrescription();
        return; // Don't close the panel yet, let the save complete
      }
    }
    setShowMedicineDictation(!showMedicineDictation);
  };
          ...
          <div className="flex h-[calc(100vh-200px)]">
            ...
            {showMedicineDictation && (
              <div className="w-96 bg-white border-l border-gray-200 overflow-y-auto">
                ...
                {/* Remove the separate Confirm Prescription Button since it's now inside MedicineDictation */}
              </div>
            )}
          </div>
